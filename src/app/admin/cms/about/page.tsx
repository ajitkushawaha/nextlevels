...
export default function AboutCMSPage() {
  ...
  // About page editable states
  const [aboutHeading, setAboutHeading] = useState('Who We Are')
  const [aboutText, setAboutText] = useState('We are a premier educational consulting agency focused on clearing paths for international students. Our counselors work round-the-clock to find courses suited for your aspirations.')

  const handleSave = () => {
    toast.success('About page CMS content saved (Mock)')
  }

  const handleReset = () => {
    setAboutHeading('Who We Are')
    setAboutText('We are a premier educational consulting agency focused on clearing paths for international students. Our counselors work round-the-clock to find courses suited for your aspirations.')
    ...
    <div className="p-6 md:p-8 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Edit about page content sections and see updates live.</p>
        </div>
      </div>
        ...
        <div className="xl:col-span-5">
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-slate-100 bg-slate-50/50 p-5">
              <CardTitle className="text-sm font-bold text-slate-800">About Content Settings</CardTitle>
            ...
            <CardContent className="p-5 space-y-5 text-xs">
              <div className="space-y-1.5">
                <Label htmlFor="about-heading">Section Title</Label>
                <Input
                  id="about-heading"
                  value={aboutHeading}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="about-text">About Description Text</Label>
                <Textarea
                  id="about-text"
                  value={aboutText}
                ...
                <div className="w-full max-w-[270px] bg-white border border-slate-300 rounded-3xl shadow-xl p-3 flex flex-col justify-between aspect-9/16 relative transition-all duration-300">
                  ...
                  <div className="flex-1 py-8 flex flex-col justify-center text-center space-y-4 px-2">
                    <div className="space-y-1">
                      <h2 className="text-base font-extrabold text-slate-950 leading-tight">{aboutHeading || 'Section Title'}</h2>
                      <span className="h-0.5 w-8 bg-[#d7a23a] rounded mx-auto block"></span>
                    </div>
                    <p className="text-[10px] text-slate-500 leading-relaxed font-medium line-clamp-6">{aboutText || 'About Text'}</p>
                  </div>
                ...
                <div className="w-full bg-white border border-slate-300 rounded-2xl shadow-xl p-4 flex flex-col justify-between aspect-16/10 transition-all duration-300">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3 px-2">
                    ...
                    <nav className="flex items-center gap-3 text-[10px] text-slate-500 font-semibold">
                      <span>Home</span>
                      <span className="text-slate-950 font-bold">About</span>
                      <span>Contact</span>
                  ...
                  <div className="flex-1 py-8 flex flex-col justify-center items-center text-center space-y-4 px-12">
                    <div className="space-y-1">
                      <h2 className="text-xl font-extrabold text-slate-950 leading-tight">{aboutHeading || 'Section Title'}</h2>
                      <span className="h-0.5 w-10 bg-[#d7a23a] rounded mx-auto block"></span>
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed font-medium max-w-lg line-clamp-5">{aboutText || 'About Text'}</p>
                  </div>
                </div>
              )}
            </CardContent>
